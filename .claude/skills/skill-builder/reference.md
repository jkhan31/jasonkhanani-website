...
Claude Code uses a 3-level loading system to manage context efficiently. Understanding this is critical for designing scalable skills.

- **Level 1: Descriptions**  
  The `description` field from the skill's frontmatter is always loaded into context. This allows Claude to know which skills are available and auto-detect when one might be useful.
  > [!IMPORTANT]
  > Keep descriptions concise and keyword-rich to stay within the character budget (see character budget section).

- **Level 2: The Brain (SKILL.md)**  
  The full content of `SKILL.md` is only loaded when the skill is explicitly or automatically invoked. This is where your core workflow and task-specific instructions live.
  > [!TIP]
  > Keep `SKILL.md` under 500 lines. Use Level 3 for detailed material.

- **Level 3: Supporting Context**  
  Additional files in the skill directory (reference docs, templates, scripts) are **not** loaded automatically. Claude only reads them if they are referenced in `SKILL.md` or if Claude explicitly needs it to perform a task.
...
| | CLAUDE.md | Skill |
|---|---|---|
| **Level** | Always Level 1 | Levels 1, 2, or 3 |
| **When loaded** | Every conversation, always | Based on invocation |
| **What it's for** | Project-wide rules, conventions | Task-specific SOPs |
| **Examples** | Coding style, test commands | "Audit PR", "Generate Report" |
...
- The skill contains guidelines (like "use these API conventions") without a concrete task. The subagent receives guidelines but no actionable prompt and returns nothing useful.
...
The word can appear anywhere -- in a comment, a heading, or inline text. Its presence alone activates extended thinking for that skill's execution.
...
1. **Check the description** -- Does it include keywords users would naturally say? The description is how Claude decides to load the skill.
...
- The skill probably contains guidelines without a concrete task. Subagents need an actionable prompt, not just reference material.
