...
const DetailItem: React.FC<{ text: string }> = ({ text }) => {
  ...
  if (parts.length >= 3) {
    return (
      <p className="text-base text-darkText/80 leading-relaxed border-b border-darkText/5 pb-3 last:border-0">
        <strong className="text-darkText/60 font-semibold tracking-wide text-xs uppercase block mb-1">
          {parts[1]}
        </strong>
        {parts[2]}
      </p>
  ...
  return <p className="text-base text-darkText/80 leading-relaxed border-b border-darkText/5 pb-3 last:border-0">{text}</p>;
};
...
const CaseCard: React.FC<{ study: CaseStudy }> = ({ study }) => {
  ...
  return (
    <div className="group border-0.5 border-warmAmber/20 bg-white rounded-xl p-6 md:p-8 hover:border-warmAmber/40 hover:bg-warmAmber/[0.015] transition-all duration-500 hover:shadow-lg">

      <div className="flex flex-col h-full">
        {/* Icon and Title Section */}
        <div className="mb-5">
          <div className="flex justify-end mb-3">
            <span className="text-[10px] uppercase tracking-widest font-bold text-darkText/40 group-hover:text-warmAmber transition-colors">
              {study.stealthTitle || 'Platform Initiative'}
            </span>
          </div>
          <div className="flex items-center gap-3 mb-2">
            <IconComponent className="w-6 h-6 text-warmAmber/80 shrink-0" strokeWidth={1.5} />
            <h3 className="text-2xl font-sans font-bold text-darkText leading-snug">
              {study.title}
            </h3>
          </div>
        </div>
        ...
        {/* Diagnosis Section */}
        {study.diagnosis && (
          <div className="mb-5 pb-4 border-b border-warmAmber/10">
            <p className="text-sm italic text-darkText/70 leading-relaxed">
              <span className="font-semibold text-darkText/80">The Challenge:</span> {study.diagnosis}
            </p>
          </div>
        )}
        ...
        {/* Learning Section */}
        {study.learned && (
          <div className="mb-3 pt-3 border-t border-darkText/5">
            <p className="text-xs text-darkText/60 leading-relaxed italic">
              {study.learned}
            </p>
          </div>
        )}
        ...
        {/* Impact Section */}
        <div className="pt-3 border-t-0.5 border-warmAmber/10">
          <span className="text-[11px] uppercase tracking-widest font-semibold text-darkText/50 mb-2 block">
            Impact
          </span>
          <span className="text-2xl font-sans font-bold text-warmAmber">{study.impact}</span>
        </div>
      </div>
    </div>
  );
